...
async def test_runusage_as_tuple(the_session):
    ...
    assert found == (1, 2, 3, 4, 5, "gpt-4o-2024-11-20", 6, usage.created)
    assert found.measured_at is not None
...
async def test_runusage_as_tuple_wo_final_request(the_session):
    ...
    assert usage.as_tuple() == (1, 2, 3, 4, None, None, None, usage.created)
...
def test_get_session(ge):
    with agui_schema.get_session() as session:
...
async def test_get_async_engine(ca, cae):
    found = await agui_schema.get_async_engine()
...
async def test_get_async_session(gae):
    session_maker = await agui_schema.get_async_session()

"""Tests for soliplex.ingester.agentsscm exception classes."""

import pytest

from soliplex.ingester.agents import ValidationError
...
def test_unexpected_response_error():
    ...
    with pytest.raises(
        UnexpectedResponseError, match="Unexpected response status"
    ):

"""Tests for soliplex.ingester.agents.scm.base module."""
...
from soliplex.ingester.agents.scm import APIFetchError
from soliplex.ingester.agents.scm import SCMException
from soliplex.ingester.agents.scm.base import BaseSCMProvider
...
async def test_paginate_single_page(provider, mock_response):
    ...
    with patch.object(provider, "get_session") as mock_get_session:
        ...
        url_template = (
            "https://api.example.com/repos/{owner}/{repo}/items?page={page}"
        )
        result = await provider.paginate(
...
@pytest.mark.asyncio
async def test_list_issues_without_comments(
    provider, mock_response, sample_issue
):
...
@pytest.mark.asyncio
async def test_list_issues_with_comments(
    provider, mock_response, sample_issue, sample_comment
):
    """Test list_issues with comments."""
    with patch.object(provider, "paginate") as mock_paginate:
        with patch.object(
            provider, "list_repo_comments"
...
@pytest.mark.asyncio
async def test_get_data_from_url_single_file(
    provider, mock_response, sample_file_record
):
    ...
    with patch.object(
        provider, "get_file_content", return_value=sample_file_record
    ):
...
@pytest.mark.asyncio
async def test_get_data_from_url_single_file_without_owner_repo(
    provider, mock_response, sample_file_record
):
    ...
    result = await provider.get_data_from_url(
        "https://api.example.com/file", mock_session
    )
...
@pytest.mark.asyncio
async def test_get_data_from_url_directory(
    provider, mock_response, sample_file_record
):
...
@pytest.mark.asyncio
async def test_get_data_from_url_directory_with_recursion(
    provider, mock_response, sample_file_record
):

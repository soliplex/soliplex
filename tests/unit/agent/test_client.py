"""Tests for soliplex.ingester.agentsclient module."""
...
from soliplex.ingester.agents import client
from soliplex.ingester.agents.scm import UnexpectedResponseError
...
async def test_post_request_success(mock_response):
    ...
    with patch(
        "soliplex.ingester.agents.client.get_session"
    ) as mock_get_session:
        mock_session = MagicMock()
        mock_session.post.return_value = create_async_context_manager(
            mock_resp
        )
        ...
        result = await client._post_request(
            "/test", form_data, expected_status=201
        )
...
async def test_create_batch():
    ...
    with patch("soliplex.ingester.agents.client._post_request") as mock_post:
...
async def test_find_batch_for_source_multiple_found(mock_response, caplog):
    ...
    with patch(
        ...
        assert result == 10
        assert (
            "Multiple batches found 3 for source github using first one"
            in caplog.text
        )
...
async def test_find_batch_for_source_url_construction(mock_response):
    ...
    with patch(
        ...
        with patch(
            "soliplex.ingester.agents.client._build_url"
        ) as mock_build_url:
            mock_build_url.return_value = "http://127.0.0.1:8000/api/v1/batch/"

            mock_session = MagicMock()
            mock_session.get.return_value = create_async_context_manager(
                mock_resp
            )
            mock_get_session.return_value = create_async_context_manager(
                mock_session
            ...
            mock_session.get.assert_called_once_with(
                "http://127.0.0.1:8000/api/v1/batch/"
            )

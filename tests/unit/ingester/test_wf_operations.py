...
async def test_create_run_group_with_invalid_batch(monkeypatch, mock_engine):  # noqa F811
    ...
    with pytest.raises(wf_ops.NotFoundError, match="Batch .* not found"):
        await wf_ops.create_run_group(
            workflow_definition_id="batch", batch_id=99999, param_id="test1"
        )
...
async def test_get_run_group(monkeypatch, mock_engine):  # noqa F811
    ...
    assert (
        retrieved_group.workflow_definition_id
...
async def test_get_run_groups_for_batch(monkeypatch, mock_engine):  # noqa F811
    ...
    batch_id = await doc_ops.new_batch("test_source", "Test Batch")
    run_group1 = await wf_ops.create_run_group(
        workflow_definition_id="batch", batch_id=batch_id, param_id="test1"
    )
    run_group2 = await wf_ops.create_run_group(
        workflow_definition_id="batch", batch_id=batch_id, param_id="test2"
    )
...
async def test_create_workflow_run(monkeypatch, mock_engine):  # noqa F811
    ...
    workflow_run, steps = await wf_ops.create_workflow_run(
        run_group=run_group, doc_id=doc.hash, priority=5
    )
...
async def test_get_workflow_run(monkeypatch, mock_engine):  # noqa F811
    ...
    workflow_run, steps = await wf_ops.create_workflow_run(
        run_group=run_group, doc_id=doc.hash
    )

    # Get workflow run without steps
    retrieved_run = await wf_ops.get_workflow_run(
        workflow_run.id, get_steps=False
    )
    ...
    retrieved_run2, retrieved_steps = await wf_ops.get_workflow_run(
        workflow_run.id, get_steps=True
    )
...
async def test_get_workflow_run_not_found(monkeypatch, mock_engine):  # noqa F811
    ...
    with pytest.raises(
        wf_ops.NotFoundError, match="workflow run .* not found"
    ):
...
async def test_get_workflows_with_steps(monkeypatch, mock_engine):  # noqa F811
    ...
    workflows_with_steps, total = await wf_ops.get_workflows(
        batch_id, include_steps=True
    )
    ...
    assert our_workflow is not None, (
        f"Could not find workflow_run.id={workflow_run.id} in results"
    )
    ...
    workflows_without_steps, total_without = await wf_ops.get_workflows(
        batch_id, include_steps=False
    )
...
async def test_get_workflows_for_status(monkeypatch, mock_engine):  # noqa F811
    ...
    pending_workflows = await wf_ops.get_workflows_for_status(
        RunStatus.PENDING, batch_id
    )
    assert len(pending_workflows) >= 1

    # Get workflows with PENDING status (no batch filter)
    all_pending = await wf_ops.get_workflows_for_status(
...
async def test_get_step_config_for_workflow_run(monkeypatch, mock_engine):  # noqa F811
    ...
    parse_config = await wf_ops.get_step_config_for_workflow_run(
        workflow_run.id, WorkflowStepType.PARSE
    )
...
@pytest.mark.asyncio
async def test_get_step_config_for_workflow_run_not_found(
    monkeypatch, mock_engine
):  # noqa F811
    """Test get_step_config_for_workflow_run with non-existent workflow"""
    do_monkeypatch(monkeypatch, mock_engine)

    with pytest.raises(wf_ops.NotFoundError, match="step config .* not found"):
        await wf_ops.get_step_config_for_workflow_run(
            99999, WorkflowStepType.PARSE
        )
...
async def test_find_operator_for_workflow_run(monkeypatch, mock_engine):  # noqa F811
    ...
    operator = await wf_ops.find_operator_for_workflow_run(
        ...
        models.ArtifactType.PARSED_JSON,
    )

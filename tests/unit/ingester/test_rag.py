...
def test_build_docling_config(mock_app_config, mock_settings):
    ...
    with patch(
        "soliplex.ingester.lib.rag.get_settings", return_value=mock_settings
    ):
        config_dict = {}
        ...
        assert (
            result.providers.docling_serve.base_url == "http://localhost:5004"
        )
...
def test_build_chunk_config_missing_required_key(
    mock_app_config, mock_settings
):
...
def test_build_storage_config_missing_required_key(
    mock_app_config, mock_settings
):
...
def test_build_full_config(mock_app_config, mock_settings):
    ...
    with patch(
        ...
        result = rag.build_full_config(
            mock_app_config, chunk_config, embed_config, storage_config
        )
...
async def test_get_chunk_objs():
    ...
    with (
        patch(
            "soliplex.ingester.lib.rag.build_chunk_config"
...
async def test_embed(mock_settings):
    ...
    with patch(
        ...
        with (
            patch(
                "soliplex.ingester.lib.rag.build_embed_config"
            ) as mock_build_config,
            patch(
                "soliplex.ingester.lib.rag.embed_chunks"
...
async def test_save_to_rag():
    ...
    with (
        patch(
            "soliplex.ingester.lib.rag.build_embed_config"
        ) as mock_build_embed_config,
        patch(
            "soliplex.ingester.lib.rag.build_storage_config"
        ) as mock_build_storage_config,
        patch(
            "soliplex.ingester.lib.rag.DoclingDocument"
        ...
        mock_docling_class.model_validate_json.return_value = (
        ...
        mock_haiku_rag.return_value.__aenter__ = AsyncMock(
        ...
        # Verify the correct functions were called
        mock_build_embed_config.assert_called_once_with(
            HRConfig, embed_config.config_json
        )
        mock_build_storage_config.assert_called_once()
        mock_docling_class.model_validate_json.assert_called_once_with(
            docling_json
        )

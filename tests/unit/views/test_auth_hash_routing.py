...
async def test_get_auth_system_with_hash_routing():
    ...
    request.url_for = Mock(return_value=Mock(replace_query_params=Mock(return_value="http://test/auth/system")))
    ...
    with patch('soliplex.auth.get_oauth', return_value=oauth), \

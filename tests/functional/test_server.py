...
def test_client():
    ...
    with patch(
        "soliplex.ingester.lib.wf.runner.start_worker", new_callable=AsyncMock
    ):
...
def test_source_status(test_client):
    ...
    with patch(
        "soliplex.ingester.server.operations.get_doc_status"
    ) as mock_status:
...
def test_get_batches(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.batch.operations.list_batches"
    ) as mock_list:
...
def test_create_batch(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.batch.operations.new_batch"
    ) as mock_new:
...
def test_start_workflows_success(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.batch.wf_ops.create_workflow_runs_for_batch"
    ) as mock_create:
        mock_run_group = Mock()
        mock_runs = [Mock(), Mock()]
        mock_create.return_value = (mock_run_group, mock_runs)
        response = test_client.post(
            "/api/v1/batch/start-workflows", data={"batch_id": 1}
        )
        assert response.status_code == 201
...
def test_start_workflows_not_found(test_client):
    ...
    with patch(
        ...
        response = test_client.post(
            "/api/v1/batch/start-workflows", data={"batch_id": 999}
        )
...
def test_batch_status_success(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.batch.operations.get_batch"
    ) as mock_get_batch:
        with patch(
            "soliplex.ingester.server.routes.batch.operations.get_documents_in_batch"
        ) as mock_get_docs:
            with patch(
                "soliplex.ingester.server.routes.batch.wf_ops.get_workflows"
            ) as mock_get_wf:
                mock_batch = Mock()
...
def test_get_docs_by_source(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.document.operations.get_uris_for_source"
    ) as mock_get:
...
def test_get_docs_by_batch(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.document.operations.get_uris_for_batch"
    ) as mock_get:
        mock_get.return_value = []
...
def test_ingest_document_success(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.document.workflow.initial_load"
    ) as mock_load:
        mock_uri = Mock()
        mock_uri.id = 1
        mock_doc = Mock()
...
def test_get_run_group_durations(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.stats.wf_ops.get_run_group_durations"
    ) as mock_get:
        mock_get.return_value = []
...
def test_get_run_group_step_stats(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.stats.wf_ops.get_step_stats"
    ) as mock_get:
        mock_get.return_value = []
...
def test_get_workflows(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_ops.get_workflows"
    ) as mock_get:
        mock_get.return_value = []
...
def test_get_workflows_for_status(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_ops.get_workflows_for_status"
    ) as mock_get:
        mock_get.return_value = []
        response = test_client.get(
            "/api/v1/workflow/by-status?status=completed"
        )
        assert response.status_code == 200
...
def test_list_workflows_definitions(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_registry.load_registry"
    ) as mock_load:
        mock_wf1 = Mock()
        mock_wf1.id = "wf1"
...
def test_get_workflow_def_success(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_registry.load_workflow_registry"
    ) as mock_load:
        mock_wf = Mock()
...
def test_list_params(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_registry.load_param_registry"
    ) as mock_load:
        mock_param = Mock()
        mock_param.id = "p1"
...
def test_get_param_set_by_target(test_client):
    ...
    with patch(
        ...
        mock_load.return_value = {"p1": mock_param}
        response = test_client.get(
            "/api/v1/workflow/param_sets/target//test/dir"
        )
        assert response.status_code == 200
...
def test_get_workflow_status(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_ops.get_run_steps"
    ) as mock_get:
        mock_get.return_value = []
...
def test_get_workflow_run_groups(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_ops.get_run_groups_for_batch"
    ) as mock_get:
        mock_get.return_value = []
...
def test_get_workflow_run_group(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_ops.get_run_group"
    ) as mock_get:
        mock_get.return_value = {}
...
def test_get_run_group_stats(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_ops.get_run_group_stats"
    ) as mock_get:
        mock_get.return_value = {}
...
def test_get_workflow_runs(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_ops.get_workflow_runs"
    ) as mock_get:
        mock_get.return_value = []
...
def test_get_workflow_by_id(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_ops.get_workflow_run"
    ) as mock_get:
        mock_get.return_value = {}
...
def test_start_workflow(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_ops.create_single_workflow_run"
    ) as mock_create:
        mock_create.return_value = {}
        response = test_client.post(
            "/api/v1/workflow/", data={"doc_id": "hash123"}
        )
        assert response.status_code == 201
...
def test_retry_workflow(test_client):
    ...
    with patch(
        "soliplex.ingester.server.routes.workflow.wf_ops.reset_failed_steps"
    ) as mock_reset:

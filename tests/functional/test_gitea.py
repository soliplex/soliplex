"""Tests for soliplex.ingester.agentsscm.gitea module."""

from unittest.mock import patch

import pytest

from soliplex.ingester.agents.scm.gitea import GiteaProvider
...
async def test_list_repo_files(gitea_provider, mock_response, mock_settings):
    ...
    api_response = [
        {
            "name": "file1.md",
            "type": "file",
            "url": "https://gitea.example.com/file1",
        },
    ]
    ...
    with patch.object(gitea_provider, "get_session") as mock_get_session:
        ...
        with patch.object(gitea_provider, "validate_response"):
            with patch.object(
                gitea_provider, "get_data_from_url"
            ) as mock_get_data:
...
async def test_paginate_integration(gitea_provider, mock_response):
    ...
    with patch.object(gitea_provider, "get_session") as mock_get_session:
        ...
        url_template = gitea_provider.build_url(
            "/repos/{owner}/{repo}/issues?page={page}"
        )
        result = await gitea_provider.paginate(

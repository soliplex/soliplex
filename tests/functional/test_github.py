"""Tests for soliplex.ingester.agentsscm.github module."""
...
from soliplex.ingester.agents.scm import GitHubAPIError
from soliplex.ingester.agents.scm import SCMException
from soliplex.ingester.agents.scm.github import GitHubProvider
...
@pytest.mark.asyncio
async def test_validate_response_non_200_with_message(
    github_provider, mock_response
):
...
@pytest.mark.asyncio
async def test_validate_response_non_200_without_message(
    github_provider, mock_response
):
...
@pytest.mark.asyncio
async def test_validate_response_with_errors_field(
    github_provider, mock_response
):
...
async def test_get_file_content_with_content(github_provider):
    ...
    result = await github_provider.get_file_content(
        rec, session, "owner", "repo"
    )
...
@pytest.mark.asyncio
async def test_get_file_content_missing_content(
    github_provider, mock_response
):
...
async def test_get_blob(github_provider, mock_response):
    ...
    from tests.unit.agent.conftest import create_async_context_manager
    ...
    result = await github_provider.get_blob(
        "test_repo", "test_owner", rec, session
    )
...
async def test_list_repo_files(github_provider, mock_response, mock_settings):
    ...
    api_response = [
        {
            "name": "file1.md",
            "type": "file",
            "url": "https://api.github.com/file1",
        },
    ]
    ...
    with patch.object(github_provider, "get_session") as mock_get_session:
        ...
        mock_get_session.return_value = create_async_context_manager(
            mock_session
        )

        with patch.object(github_provider, "validate_response"):
            with patch.object(
                github_provider, "get_data_from_url"
            ) as mock_get_data:
...
async def test_inheritance_from_base(github_provider):
    ...
    from soliplex.ingester.agentsscm.base import BaseSCMProvider
...
async def test_paginate_integration(github_provider, mock_response):
    ...
    with patch.object(github_provider, "get_session") as mock_get_session:
        ...
        url_template = github_provider.build_url(
            "/repos/{owner}/{repo}/issues?page={page}"
        )
        result = await github_provider.paginate(
            url_template, "test_owner", "test_repo"
        )
...
@pytest.mark.asyncio
async def test_get_data_from_url_with_empty_content(
    github_provider, mock_response
):

...
def init():
    try:
        ...
        print(
            "invalid settings. environment variables might not be set.  Run `si-cli validate-settings`"
        )
...
def export_to_env(file_path: str):
    ...
    if os.path.exists(file_path):
        print(
            f"{file_path} already exists. remove or choose a different path."
        )
        return
...
def init_haiku():
    ...
    if os.path.exists("haiku.rag.yaml"):
        print(
            "haiku.rag.yaml already exists. remove or choose a different path."
        )
        return
...
def init_config():
    ...
    shutil.copyfile(
        Path(ex.__file__).parent / "default_wf.yaml",
        wf_path / "default_wf.yaml",
    )
    shutil.copyfile(
        Path(ex.__file__).parent / "default_params.yaml",
        param_path / "default.yaml",
    )
...
def serve(
    ...
    proxy_headers: bool = typer.Option(
        ...
        "--proxy-headers",
        help=(
            "Enable/Disable X-Forwarded-Proto, X-Forwarded-For to populate url scheme and remote address info."
        ),
    ),

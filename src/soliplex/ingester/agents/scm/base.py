...
from soliplex.ingester.agents.config import settings
from soliplex.ingester.agents.scm import APIFetchError
from soliplex.ingester.agents.scm import SCMException
from soliplex.ingester.agents.scm.lib.utils import compute_file_hash
from soliplex.ingester.agents.scm.lib.utils import decode_base64_if_needed
from soliplex.ingester.agents.scm.lib.utils import flatten_list
...
class BaseSCMProvider(ABC):
    ...
    async def list_issues(
        self, repo: str, owner: str | None = None, add_comments: bool = False
    ) -> list[dict[str, Any]]:
        """
        ...
        url_template = self.build_url(
            "/repos/{owner}/{repo}/issues?page={page}&status=all"
        )
    ...
    async def list_repo_comments(
        ...
        owner = owner or self.owner
        url_template = self.build_url(
            "/repos/{owner}/{repo}/issues/comments?page={page}"
        )
        ...
        try:
            async with session.get(url) as response:
                ...
                if isinstance(res, dict):
                    ...
                    if owner and repo:
                        res = await self.get_file_content(
                            res, session, owner, repo
                        )
                    ...
                    for r in res:
                        logger.debug(
                            f"fetching file in dir for url = {r['url']}"
                        )
                        parsed.append(
                            await self.get_data_from_url(
                                r["url"], session, owner, repo
                            )
                        )
                    return parsed
    ...
    async def validate_response(
        self, response: aiohttp.ClientResponse, resp: dict | list
    ) -> None:
        """

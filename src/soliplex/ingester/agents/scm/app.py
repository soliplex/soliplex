...
async def get_data(scm: str, repo_name: str, owner: str = None):
    ...
    files = await impl.list_repo_files(
        repo_name, owner, allowed_extensions=allowed_extensions
    )

    issues = await impl.list_issues(
        repo=repo_name, owner=owner, add_comments=True
    )
    doc_data = []
    filtered_files = [
        x for x in files if x["name"].split(".")[-1] in allowed_extensions
    ]

...
async def get_workflows(
    ...
    list[WorkflowRun]
    | list[WorkflowRunWithSteps]
    | PaginatedResponse[WorkflowRun]
    | PaginatedResponse[WorkflowRunWithSteps]
):
    """
    ...
    total_pages = (
        (total + rows_per_page - 1) // rows_per_page
        if rows_per_page > 0
        else 0
    )
...
async def get_workflows_for_status(
    ...
    items, total = await wf_ops.get_workflows_for_status(
        status, batch_id, page=page, rows_per_page=rows_per_page
    )
...
async def get_workflow_def(workflow_id: str, response: Response):
    ...
    if workflow_id not in wf:
        ...
        return {
            "error": f"workflow definition {workflow_id} not found {wf.keys()}"
        }
...
async def get_param_set_by_target(
    target: str, response: Response
) -> list[WorkflowParams]:
...
@wf_router.get(
    "/steps", status_code=status.HTTP_200_OK, summary="get run steps by status"
)
...
async def get_workflow_run_groups(
    response: Response, batch_id: int | None = None
) -> list[RunGroup]:
    try:
...
async def start_workflow(
    ...
    try:
        return await wf_ops.create_single_workflow_run(
            workflow_definiton_id, doc_id, priority=priority, param_id=param_id
        )
    except Exception as e:
        response.status_code = status.HTTP_500_INTERNAL_SERVER_ERROR

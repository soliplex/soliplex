...
async def _get_op(
    ...
    op = await wf_ops.find_operator_for_workflow_run(
        workflow_run_id, step_type, artifact_type
    )
    return op
...
async def validate_document(
    ...
    """
    logger.info(
        f"validate_document started  source={source} batch_id={batch_id} doc_hash={doc_hash}"
    )
    doc = await doc_ops.get_document(doc_hash)
    if doc:
        logger.info(
            f"validate_document found  source={source} batch_id={batch_id} doc_hash={doc_hash}"
        )
    else:
    ...
    if "is_valid" not in meta:
        ...
        if doc.mime_type == "application/pdf":
            try:
                file_bytes = await doc_ops.read_doc_bytes(
                    doc_hash, ArtifactType.DOC
                )
    ...
    logger.info(
        f"validate_document found valid  source={source} batch_id={batch_id} doc_hash={doc_hash}"
    )
    return
...
async def split_parse_document(
    ...
    _lc = log_context(
        doc_hash=doc_hash, batch_id=batch_id, action="split_parse"
    )
    logger.info(
        f"parse_document started  {source} {batch_id} {doc_hash}", extra=_lc
    )
    ...
    if len(doc_uris) == 0:
        logger.warning(
            ...
            extra=log_context(
                doc_hash=doc_hash, batch_id=batch_id, action="do_parse"
            ),
        )
        return
    test_op = await _get_op(
        workflow_run.id, WorkflowStepType.PARSE, ArtifactType.PARSED_JSON
    )
    ...
    async with aiofiles.tempfile.TemporaryDirectory() as temp_dir:
        ...
        logger.info(
            f"starting  split_to_files file_Size={len(file_bytes)}", extra=_lc
        )
        ...
        if len(split_result) == 2:
            ...
            logger.info(
                f"document {doc_hash} size={len(file_bytes)} split into {len(split_files)} "
            )
            if use_serve:
                logger.info(f"parse_document {doc_hash} using serve")
                byte_list = await asyncio.gather(
                    *[read_file(x) for x in split_files]
                )
                ...
                start = time.time()
                logger.info(
                    f"parse_document {doc_hash} using batch processing workers={split_workers} "
                )
            ...
            mdop = await _get_op(
                workflow_run.id, WorkflowStepType.PARSE, ArtifactType.PARSED_MD
            )
            await mdop.write(doc_hash, markdown_txt.encode("utf-8"))
            await doc_ops.add_history_for_hash(
                doc_hash, "parsed", batch_id=batch_id
            )
        else:
            msg = f"split_to_files returned {split_result}.  should return 2 files.  {source_uri} {doc_hash}"
            raise WorkflowException(msg)
    logger.info(
        f"parse_document completed  {source} {batch_id} {doc_hash}", extra=_lc
    )
...
async def chunk_document(
    ...
    op = await _get_op(
        workflow_run.id, WorkflowStepType.CHUNK, ArtifactType.CHUNKS
    )
    ...
    if not exists or force:
        ...
        chunk_objs = await rag.get_chunk_objs(
            docling_document, step_config.config_json
        )
        ...
        await doc_ops.add_history_for_hash(
            doc_hash, "chunked", batch_id=batch_id
        )
...
async def embed_document(
    ...
    logger.info(
        f"embed_document started  {source} {batch_id} {doc_hash}", extra=_lc
    )
    chunk_op = await _get_op(
        workflow_run.id, WorkflowStepType.CHUNK, ArtifactType.CHUNKS
    )
    ...
    embed_chunks = await rag.embed(
        chunk_objs, step_config.config_json, doc_hash=doc_hash
    )
    embed_op = await _get_op(
        workflow_run.id, WorkflowStepType.EMBED, ArtifactType.EMBEDDINGS
    )
    ...
    logger.info(
        f"embed_document completed  {source} {batch_id} {doc_hash}", extra=_lc
    )
...
async def save_to_rag(
    ...
    r = asyncio.gather(
        ...
        wf_ops.get_step_config_for_workflow_run(
            workflow_run.id, WorkflowStepType.EMBED
        ),
    )
    ...
    await doc_ops.add_history_for_hash(
        doc_hash, "ingested", hist_meta={"haiku_id": rag_id}, batch_id=batch_id
    )

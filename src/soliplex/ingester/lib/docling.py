...
SMALLEST_PNG = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg=="
...
@retry(
    stop=stop_after_attempt(4), wait=wait_exponential_jitter(), reraise=True
)
async def docling_convert(
    ...
    async with _http_sem:
        ...
        _async_client = httpx.AsyncClient(
            timeout=env.docling_http_timeout, cookies=local_jar
        )
        ...
        if "ocr_lang" in config_dict and isinstance(
            config_dict["ocr_lang"], str
        ):
        ...
        response = await _async_client.post(
            async_url, files=files, data=parameters
        )
                ...
                async for msg in ws:
                    if msg.type == aiohttp.WSMsgType.TEXT:
                        ...
                        if payload["message"] == "error":
                            break
                        if payload["message"] == "update" and payload["task"][
                            "task_status"
                        ] in (
                            "success",
                            "failure",
        ...
        if (
            "task" in payload
            and "task_status" in payload["task"]
            and payload["task"]["task_status"] == "failure"
        ):
        ...
        logger.info(
            f"{task_id} result={res['status']} processing time={res['processing_time']}"
        )
        ...
        if res["status"] == "success":
            ...
            for output_format in output_formats:
                ...
                if output_format == "json":
                    if (
                        "image_export_mode" in parameters
                        and parameters["image_export_mode"] == "placeholder"
                    ):
                        logger.info(
                            f" doing placeholder replacement for {source_uri}"
                        )
                        output_content = do_repl(output_content)
                    parsed[output_format] = json.dumps(output_content).encode(
                        "utf-8"
                    )
                else:

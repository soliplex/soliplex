...
async def set_step_status(
    ...
    try:
        async with get_session() as session:
            ...
            step = results.first()
            step.status = do_state_stransition(
                step.status, status, step.retry, step.retries, step.worker_id
            )
            ...
            if increase_retry:
                if step.retry >= step.retries:
                    logger.warning(
                        f"step {step.id} already failed {step.retry} times"
                    )
                step.retry += 1
            ...
            await operations.update_run_status(
                step.workflow_run_id, step.is_last_step, step.status, session
            )
...
async def get_runnable_steps(
    top: int | None = None, batch_id: int | None = None
) -> list[RunStep]:
    if top is None:
        top = 100
    async with get_session() as session:
        batch_filt = (
            f" and r.batch_id={batch_id}" if batch_id is not None else ""
        )
        ...
        if res:
            ...
            for r in res:
                ...
                model_dict["step_type"] = WorkflowStepType[
                    model_dict["step_type"]
                ]
...
def get_lifecycle_event(
    workflow_def: WorkflowDefinition, evt: LifeCycleEvent
...
async def handle_lifecycle_event(
    ...
    if run_step.status == RunStatus.COMPLETED:
        ...
        if run_step.is_last_step:
            ...
            stats = await operations.get_run_group_stats(run_group.id)
            if (
                stats[RunStatus.RUNNING] == 0
                and stats[RunStatus.PENDING] == 0
                and stats[RunStatus.ERROR] == 0
            ):
                await run_lifecycle_event(
                    workflow_def,
...
async def run_lifecycle_event(
    ...
    if evt_list is not None and len(evt_list) > 0:
        try:
            ...
            for evt in evt_list:
                logger.info(
                    f"executing lifecycle event {evt.name} for {workflow_def.name}"
                )
            ...
            logger.exception(
                f"step start {workflow_run.doc_id} failed", exc_info=e
            )
            try:
                await operations.update_lifecycle_history(
                    run_group.id,
...
async def run_wf_step(run_step: RunStep, coro_id: int = None):
    ...
    try:
        ...
        await handle_lifecycle_event(
            workflow_def, run_step, workflow_run, run_group
        )
        res = await build_step_coro(
            run_step, workflow_run, workflow_def, step_config, batch, run_group
        )
        ...
        await handle_lifecycle_event(
            workflow_def, upd_step, workflow_run, run_group
        )
...
async def consume_tasks(coro_id: int):
    ...
    while True:
        ...
        if run_step:
            logger.debug(
                f"worker cid={coro_id} running step "
                + f"{run_step.workflow_run_id} {run_step.workflow_step_number}",
                extra=lc,
            )
...
async def queue_tasks():
    ...
    while True:
        try:
            ...
            logger.debug(
                f"queue task put {task_ct} curr={_task_queue.qsize()},'worker_id {get_worker_id()}'"
            )
            task_ct += 1

...
class WebAuthPendingStorage {
  ...
  Future<void> savePendingAuth(PendingWebAuth pending) async {
    ...
    DebugLog.auth('WebAuthPendingStorage: Saved pending auth for ${pending.serverId}');
  }
  ...
  Future<PendingWebAuth?> getPendingAuth() async {
    ...
    try {
      ...
      DebugLog.auth('WebAuthPendingStorage: Retrieved pending auth for ${pending.serverId}');
      return pending;
    } catch (e) {
  ...
  /// Validate that the returned state matches the stored state (CSRF protection)
  Future<bool> validateState(String returnedState) async {
    final pending = await getPendingAuth();
    if (pending == null) {
      DebugLog.warn('WebAuthPendingStorage: Cannot validate state - no pending auth');
      return false;
    }

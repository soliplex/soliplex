...
import 'web_auth_callback_stub.dart'
    if (dart.library.js_interop) 'web_auth_callback_web.dart' as platform;
...
class WebAuthCallbackHandler {
  ...
  /// [AuthCallbackFailure] on error, or [AuthCallbackNotDetected] if not on callback URL.
  Future<AuthCallbackResult> handleCallback() async {
    ...
    DebugLog.auth('WebAuthCallbackHandler: Extracted params - code: ${code != null}, state: ${state != null}, error: $error');
    ...
    if (pending == null) {
      ...
      return AuthCallbackFailure(
        ...
        description: 'The authentication session may have expired. Please try again.',
      );
    }
    ...
    DebugLog.auth('WebAuthCallbackHandler: State validated, exchanging code for tokens...');
  ...
  Future<OidcAuthTokenResponse> _exchangeCodeForTokens({
    ...
    DebugLog.auth('WebAuthCallbackHandler: POSTing to token endpoint: $tokenEndpoint');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        DebugLog.error('WebAuthCallbackHandler: Token endpoint returned ${response.statusCode}');
        ...
        try {
          ...
          throw Exception('${errorCode ?? 'token_error'}: ${errorDesc ?? response.body}');
        } catch (_) {
...
class OidcWebRedirectException implements Exception {
  ...
  @override
  String toString() => 'OidcWebRedirectException: Redirecting to OIDC provider for $serverId';
}

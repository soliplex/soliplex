...
class CompletionsProbeResult extends Equatable {
  ...
  bool get isReady => isReachable && (availableModels.isNotEmpty || defaultModel != null);
...
class CompletionsProbe {
  ...
  /// Otherwise, probes without authentication (works for local endpoints like Ollama).
  Future<CompletionsProbeResult> probe(
    String url, {
    String? apiKey,
  }) async {
    ...
    final modelsResult = await _probeModelsEndpoint(normalizedUrl, apiKey: apiKey);
    ...
    return await _probeChatEndpoint(normalizedUrl, apiKey: apiKey);
  }
  ...
  Future<CompletionsProbeResult?> _probeModelsEndpoint(
    ...
    try {
      ...
      return CompletionsProbeResult.unreachable(baseUrl, 'Connection timed out');
    } catch (e) {
  ...
  Future<CompletionsProbeResult> _probeChatEndpoint(
    ...
    try {
      ...
      // If we get any response other than 404/network error, endpoint likely exists
      if (response.statusCode == 401) {
        ...
        // Endpoint exists (400/422 = bad request but endpoint found, 200 = weird but ok)
        return CompletionsProbeResult.success(
          url: baseUrl,
          requiresApiKey: apiKey != null,
        );
...
final completionsProbeProvider =
    Provider<CompletionsProbe>((ref) {

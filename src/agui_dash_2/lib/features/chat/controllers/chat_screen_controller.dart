...
class ChatScreenController extends AutoDisposeFamilyAsyncNotifier<void, ChatRouteParams> {
  @override
  Future<void> build(ChatRouteParams params) async {
    ...
    if (roomId != null) {
      ...
      if (currentSelected != roomId) {
        DebugLog.ui('ChatScreenController: Syncing route roomId $roomId -> provider');
        Future.microtask(() {
      ...
      // We must defer the side effect to avoid "modifying provider during build" error
      // Note: This means the UI might render before switchRoom completes, but since we pass roomId explicitly,
...
final chatScreenControllerProvider =
    AsyncNotifierProvider.autoDispose.family<ChatScreenController, void, ChatRouteParams>(

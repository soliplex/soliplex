  ...
  group('SendToCanvasUseCase', () {
    ...
    group('execute with GenUI content', () {
      test('sends GenUI widget directly to canvas', () {
        ...
        expect((result as SendToCanvasSuccess).widgetName, equals('TestWidget'));
        verify(() => mockCanvasNotifier.addItem('TestWidget', {'key': 'value'}))
            .called(1);
    ...
    group('execute with text content', () {
      test('analyzes text and sends to canvas', () {
        ...
        final useCase = SendToCanvasUseCase(ref, contentService: mockContentService);
        ...
        verify(() => mockContentService.analyze('test content', sourceMessageId: 'msg-123'))
            .called(1);
        verify(() => mockCanvasNotifier.addItem('NoteCard', {'content': 'test content'}))
            .called(1);
      });
      ...
      test('returns failure for empty content', () {
        ...
        expect((result as SendToCanvasFailure).error, equals('Content is empty'));
    ...
    group('error handling', () {
      test('returns failure when no active canvas', () {
        container = createContainer(canvasNotifier: null);
        final ref = container.read(providerContainerProvider);
        final useCase = SendToCanvasUseCase(ref, contentService: mockContentService);

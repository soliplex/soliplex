...
void main() {
  group('StorageKeys', () {
    test('serverAccessToken generates correct key', () {
      expect(StorageKeys.serverAccessToken('test-id'), 'server_test-id_access_token');
    });

    test('serverRefreshToken generates correct key', () {
      expect(StorageKeys.serverRefreshToken('test-id'), 'server_test-id_refresh_token');
    });

    test('serverTokenExpiry generates correct key', () {
      expect(StorageKeys.serverTokenExpiry('test-id'), 'server_test-id_token_expiry');
    });

    test('endpointApiKey generates correct key', () {
      expect(StorageKeys.endpointApiKey('openai-1'), 'endpoint_openai-1_api_key');
    });
  });

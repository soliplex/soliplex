...
void main() {
  group('CompletionMessage', () {
    ...
    test('deserializes from JSON', () {
      final msg = CompletionMessage.fromJson({
        'role': 'assistant',
        'content': 'Response',
      });
      expect(msg.role, 'assistant');
  ...
  group('CompletionResponse', () {
    test('deserializes from JSON', () {
      final response = CompletionResponse.fromJson({
        'id': 'chatcmpl-123',
        'model': 'gpt-4',
        'choices': [
          {
  ...
  group('CompletionsStreamParser', () {
    ...
    test('parses data line', () {
      final chunk = parser.parseLine(
        'data: {"id":"chatcmpl-123","choices":[{"index":0,"delta":{"content":"Hi"}}]}',
      );
  ...
  group('Delta', () {
    test('deserializes with content', () {
      final delta = Delta.fromJson({'content': 'Hello'});
      expect(delta.content, 'Hello');
      expect(delta.role, isNull);
    });

    test('deserializes with role', () {
      final delta = Delta.fromJson({'role': 'assistant'});

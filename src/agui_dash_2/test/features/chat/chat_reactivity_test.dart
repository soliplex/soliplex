...
void main() {
  group('Chat Reactivity Tests', () {
    ...
    setUp(() {
      ...
      container = ProviderContainer(
        overrides: [
          ...
          currentServerFromAppStateProvider.overrideWith((ref) => 
            ServerConnection.agUi(id: 'test-server', url: 'http://test-server.com', lastConnected: DateTime.now())
          ),
        ],
      );
    });
    ...
    test('activeMessageStreamProvider updates when session messages change', () async {
      ...
      if (asyncVal1.hasError) fail('Provider error: ${asyncVal1.error}');
      if (asyncVal1.isLoading && !asyncVal1.hasValue) fail('Provider stuck loading');
    ...
    test('activeMessageStreamProvider switches when active room changes', skip: 'Flaky timing in test env', () async {
      final registry = container.read(connectionRegistryProvider);
      
      const serverId = 'test-server';
      final key1 = ServerRoomKey(serverId: serverId, roomId: 'room1');
      final key2 = ServerRoomKey(serverId: serverId, roomId: 'room2');
      
      final session1 = registry.getSession(key1, baseUrl: 'http://test-server.com');
      final session2 = registry.getSession(key2, baseUrl: 'http://test-server.com');
      ...
      if (!asyncVal.hasValue) fail('Room 1: Provider has no value. Loading: ${asyncVal.isLoading}, Error: ${asyncVal.error}');
      ...
      if (!asyncVal.hasValue) fail('Room 2: Provider has no value. Loading: ${asyncVal.isLoading}, Error: ${asyncVal.error}');

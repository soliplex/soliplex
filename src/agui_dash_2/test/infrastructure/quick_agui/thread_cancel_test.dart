...
void main() {
  group('Thread cancel behavior - background result collection', () {
    test('Thread continues consuming events after cancel token fires', () async {
    ...
    test('Stream completes normally after cancel - server closes stream',
        () async {
      final events = <ag_ui.BaseEvent>[];
      final cancelToken = CancelToken();
    ...
    test('Cancel mid-stream preserves events received before and after cancel',
        () async {
      final events = <ag_ui.BaseEvent>[];
      final cancelToken = CancelToken();
      var cancelledAt = 0;
      ...
      final thread = Thread(
        ...
        runAgent: (endpoint, input) => Stream.fromIterable([
          ...
          const ag_ui.TextMessageContentEvent(messageId: 'm1', delta: 'Part 1'),
          const ag_ui.TextMessageContentEvent(messageId: 'm1', delta: 'Part 2'),
          const ag_ui.TextMessageContentEvent(messageId: 'm1', delta: 'Part 3'),
          const ag_ui.TextMessageEndEvent(messageId: 'm1'),
        ]),
      );

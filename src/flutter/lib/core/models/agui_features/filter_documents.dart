...
FilterDocuments filterDocumentsFromJson(String str) => FilterDocuments.fromJson(json.decode(str));

String filterDocumentsToJson(FilterDocuments data) => json.encode(data.toJson());
...
class FilterDocuments {
    ...
    factory FilterDocuments.fromJson(Map<String, dynamic> json) => FilterDocuments(
        documentIds: json["document_ids"] == null ? [] : List<String>.from(json["document_ids"]!.map((x) => x)),
    );

    Map<String, dynamic> toJson() => {
        "document_ids": documentIds == null ? [] : List<dynamic>.from(documentIds!.map((x) => x)),
    };
}
